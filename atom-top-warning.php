...
$myDefaultZone = 'TXZ206'; // change this to your zone

$hurlURL = 'advisories.php'; // change this default to your webpage to open for details
...
$icons_folder = './img/alert'; // No slash on end
...
if ($xml == 'invalid zone') {
      $advisory_html = "${string}${Status}<div class=\"alert alert-block\">Top Warning Information Unavailable, invalid advisory zone selected.</div>\n";
}
else if ($get_file_failed == 1) {
      $advisory_html = "${string}${Status}" . graceful_error();
}
else if ($get_file_failed == 2) {
      $advisory_html = "${string}${Status}<div class=\"alert alert-block\">Top Warning Information Unavailable, cache file conflict, view source for details.</div>";
}
else {
  ...
  if ($WLink == 0 && !preg_match("|There are no active|i", $xml->entry[0]->title)) {
      ...
      if (preg_match("| issued|", $title)) {
       ...
       // "There are no active watches, warnings or advisories for $Zone"
          ...
          $title = trim($title);
          $Summary = "<div class=\"alert alert-error\">Error: could not find the top active warning for zone $Zone, picking 1st one...<br />
  ...
  global $Zone, $WLink, $cacheName, $string, $Status, $enable_alert_icons, $icons_folder;
    ...
    if (preg_match("|There are no active|i", $title)) {
       ...
       if (!$doSilentNoActive) { // setting to supress the no active message.
          $Summary = '<div class="alert alert-success">'.$icons_warning.$title . " for zone $Zone.</div>\n";
       } else {
       ...
       return;
        ...
        if ($keyword == 'evacuation') { //0,1,2
          if (preg_match("/Evacuation Immediate/i", $title) and !preg_match("/National/i", $title)){
		    $Summary = '<div class="alert alert-error alert-error">'.$icons_warning.$this_icon.'<a href="' . $hurlURL."?zone=$Zone#WL$WLink" . '"><strong><span style="text-transform: uppercase;">' . $title .'</span></strong> In Effect  ... [Click here for more]</a></div>';
		    $WLink++;
		    break;
		  }
        } else
        ...
        if ($keyword == 'warning') { //0,1,2
          if (preg_match("/Warning/i", $title) and !preg_match("/National/i", $title) and !preg_match("/There are no/i", trim($title)) ){
		    $Summary = '<div class="alert alert-error">'.$icons_warning.$this_icon.'<a href="' . $hurlURL."?zone=$Zone#WL$WLink" . '">' . $title .' In Effect  ... [Click here for more]</a></div>';
			$WLink++;
			break;
		}
        } else
        if ($keyword == 'watch') {  //4,5
          if (preg_match("/Watch/i", $title) and !preg_match("/National/i", $title) ){
		    $Summary = '<div class="alert">'.$icons_warning.$this_icon.'<a href="' . $hurlURL."?zone=$Zone#WL$WLink" . '">' . $title .' In Effect  ... [Click here for more]</a></div>';
			$WLink++;
			break;
		}
        } else
        if ($keyword == 'advisory') { //6
          if (preg_match("/Advisory/i", $title) and !preg_match("/National/i", $title) ){
		    $Summary = '<div class="
alert alert-info">'.$icons_warning.$this_icon.'<a href="' . $hurlURL."?zone=$Zone#WL$WLink" . '">' . $title .' In Effect  ... [Click here for more]</a></div>';
			$WLink++;
			break;
		}
        } else
        if ($keyword == 'fire') {  //2
          if (preg_match("/Fire Danger/i", $title) and !preg_match("/National/i", $title) ){
		    $Summary = '<div class="alert alert-info">'.$icons_warning.$this_icon.'<a href="' . $hurlURL."?zone=$Zone#WL$WLink" . '">' . $title .' In Effect  ... [Click here for more]</a></div>';
			$WLink++;
			break;
		}
        } else
...
function graceful_error() {
  ...
  global $RSS_URL;
  $returnstring = "<div class=\"alert alert-infonoactive\">Top Warning Information Unavailable, error fetching or reading data from the <a href=\"$RSS_URL\">NOAA advisories server.</a></div>\n";
